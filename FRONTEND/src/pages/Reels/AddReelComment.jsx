...
import useCommentReel from "../../hooks/useCommentReel";
...
const AddReelComment = ({ reelId }) => {
  ...
  const { mutate: reelComment , isPending} = useCommentReel(reelId);
  

  const handleComment = () => {
    reelComment({ reelId: reelId, message: comment });
  };

  return (
    <div className="w-full relative h-13 flex items-center mb-2 px-2">
      {showPicker && (
